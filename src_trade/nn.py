...
class NeuralNetwork:
    ...
    def backward(self, x: np.ndarray, target: np.ndarray,
        ...
        """
        FIX-NN-1: uses cached z3/a2/a1/z2/z1 — does NOT re-run forward().
        The caller (TradingDQNAgent.learn) must call forward() first.
        """
        x      = np.atleast_2d(x)
        ...
        # FIX-NN-3: rebuild param list so it never goes stale
        self._params = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3]
